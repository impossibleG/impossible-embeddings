...
impl EmbeddingApplication {
    ...
    async fn load_locked(&self, manifest: &Manifest) -> Result<(), ApplicationError> {
        ...
        self.0
            ...
            .insert(manifest.canonical_id.clone());
        Ok(())
    }
    ...
    pub async fn preload(&self) -> Result<StartupReport, ApplicationError> {
        ...
        for canonical in &self.0.preload_models {
            ...
            match self.load_locked(&manifest).await {
                ...
                Err(error) => {
                    for loaded in newly_loaded.into_iter().rev() {
                        if self.0.runtime.unload(&loaded).is_ok() {
                            if let Ok(mut state) = self.0.loaded.write() {
                                state.remove(&loaded);
                            }
                        }
                    }
    ...
    pub async fn shutdown_with_timeout(&self, timeout: Duration) -> bool {
        // Serialize the application-side loaded registry with lifecycle publication. The runtime
        // independently fences late native publication, while this guard prevents a successful
        // load from recording `Loaded` after shutdown has already cleared application state.
        let _lifecycle = self.0.lifecycle.lock().await;
...
mod tests {
    ...
    use std::{fs, sync::atomic::AtomicUsize, thread};
